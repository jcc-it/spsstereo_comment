...
/******
 * 
 * SGMSTEREO_DEFAULT_DISPARITY_TOTAL: 视差总数，默认值为 256。这是用于确定视差搜索范围的参数，表示在立体匹配过程中搜索的最大视差值。视差值表示左图像中的像素在右图像中对应匹配的像素的偏移量。

SGMSTEREO_DEFAULT_DISPARITY_FACTOR: 视差因子，默认值为 256。这是用于计算视差值的缩放因子，将视差值从像素单位转换为实际的距离单位（例如毫米）。

SGMSTEREO_DEFAULT_SOBEL_CAP_VALUE: Sobel 算子的最大值，默认值为 15。Sobel 算子用于计算图像的梯度信息，帮助寻找视差匹配点。此参数用于限制梯度值的范围。

SGMSTEREO_DEFAULT_CENSUS_WINDOW_RADIUS: Census 变换的窗口半径，默认值为 2。Census 变换用于描述图像局部区域的像素值排列方式，帮助进行立体匹配。

SGMSTEREO_DEFAULT_CENSUS_WEIGHT_FACTOR: Census 变换的权重因子，默认值为 1.0/6.0。Census 变换中每个像素与周围像素的比较会产生一个二进制编码，此参数用于控制不同比较位置的权重。

SGMSTEREO_DEFAULT_AGGREGATION_WINDOW_RADIUS: 聚合窗口半径，默认值为 2。在立体匹配过程中，会对视差值进行聚合，此参数用于指定聚合时的窗口大小。

SGMSTEREO_DEFAULT_SMOOTHNESS_PENALTY_SMALL: 视差平滑度惩罚系数（较小尺度），默认值为 100。此参数用于控制在立体匹配过程中，相邻像素的视差值的平滑度。

SGMSTEREO_DEFAULT_SMOOTHNESS_PENALTY_LARGE: 视差平滑度惩罚系数（较大尺度），默认值为 1600。与上述参数相似，但用于较大尺度的平滑度控制。

SGMSTEREO_DEFAULT_CONSISTENCY_THRESHOLD: 一致性阈值，默认值为 1。在立体匹配过程中，通过对比左右图像的匹配一致性，可以提高立体匹配的准确性。此参数用于控制一致性检查的阈值。
 * ****/
// Default parameters
const int SGMSTEREO_DEFAULT_DISPARITY_TOTAL = 256; 															//视差总数
const double SGMSTEREO_DEFAULT_DISPARITY_FACTOR = 256;												//视差因子
const int SGMSTEREO_DEFAULT_SOBEL_CAP_VALUE = 15;															//Sobel 算子的最大值
const int SGMSTEREO_DEFAULT_CENSUS_WINDOW_RADIUS = 2;											  //变换的窗口半径
const double SGMSTEREO_DEFAULT_CENSUS_WEIGHT_FACTOR = 1.0/6.0;							//变换的权重因子
const int SGMSTEREO_DEFAULT_AGGREGATION_WINDOW_RADIUS = 2;									//聚合窗口半径
const int SGMSTEREO_DEFAULT_SMOOTHNESS_PENALTY_SMALL = 100;									//视差平滑度惩罚稀疏small
const int SGMSTEREO_DEFAULT_SMOOTHNESS_PENALTY_LARGE = 1600;								//视差平滑度惩罚稀疏big
const int SGMSTEREO_DEFAULT_CONSISTENCY_THRESHOLD = 1;												  //一致性阈值
...
void SGMStereo::setDisparityTotal(const int disparityTotal) {
	if (disparityTotal <= 0 || disparityTotal%16 != 0) {
		throw std::invalid_argument("[SGMStereo::setDisparityTotal] the number of disparities must be a multiple of 16");//为什么必须是16的倍数
	}
...
void SGMStereo::computeCostImage(const png::image<png::rgb_pixel>& leftImage, const png::image<png::rgb_pixel>& rightImage) {
	...
	*/
	...
	memset(leftCostImage_, 0, width_*height_*disparityTotal_*sizeof(unsigned short));//对所有的leftCostImage_ 全部置为0
