...
//***检验迭代次数是否有效，大于等于1有效，若内外部迭代均有效，则保存到内部变量中
void SPSStereo::setIterationTotal(const int outerIterationTotal, const int innerIterationTotal) {
	...
	outerIterationTotal_ = outerIterationTotal; //外部迭代次数
	innerIterationTotal_ = innerIterationTotal; //内部迭代次数
}

//***设置权重参数（位置，差距，边界框，平滑度），大于等于0有效
...
//***设置惩罚参数（惩罚参数的作用控制误差调整程度，越大越可防止过拟合）
void SPSStereo::setPenaltyParameter(const double hingePenalty, const double occlusionPenalty, const double impossiblePenalty) {
	...
	hingePenalty_ = hingePenalty;		//***铰链惩罚（不同深度估计的情况）
	occlusionPenalty_ = occlusionPenalty;		//***遮挡惩罚
	impossiblePenalty_ = impossiblePenalty;		//***处理不可能情况的惩罚
}
						...
						std::vector< std::vector<int> >& boundaryLabels)
{
	if (superpixelTotal < 2) {
		throw std::invalid_argument("[SPSStereo::compute] the number of superpixels is less than 2");//超像素：具有一定特征的不规则像素块
	}
	//***获取图像大小
...
//***为一下变量分配内存地址及相应内存大小，并对应指针变量（malloc给的内存需要free释放）
void SPSStereo::allocateBuffer() {
	inputLabImage_ = reinterpret_cast<float*>(malloc(width_*height_*3*sizeof(float)));		//lab（一种包含亮度信息的彩图）色彩空间图
	initialDisparityImage_ = reinterpret_cast<float*>(malloc(width_*height_*sizeof(float)));		//初始化视差图
	labelImage_ = reinterpret_cast<int*>(malloc(width_*height_*sizeof(int)));		//标签图像
	outlierFlagImage_ = reinterpret_cast<unsigned char*>(malloc(width_*height_*sizeof(unsigned char)));		//外点（异常值）标记图像
	boundaryFlagImage_ = reinterpret_cast<unsigned char*>(malloc(width_*height_*sizeof(unsigned char)));		//边界框标记图像
}

//***对应释放相应的指针指向的内存，避免内存泄漏
...
//***输入图像（包含对象信息），并用SGM作初始化处理
void SPSStereo::setInputData(const png::image<png::rgb_pixel>& leftImage, const png::image<png::rgb_pixel>& rightImage) {
	//***获取lab图，对于指针inputLabImage_指向地址
	setLabImage(leftImage);

	computeInitialDisparityImage(leftImage, rightImage);
}

//***从左图设置LAB图像信息，输入的是rgb图，需要lab图，最后得到lab图
void SPSStereo::setLabImage(const png::image<png::rgb_pixel>& leftImage) {
	std::vector<float> sRGBGammaCorrections(256);		//设置一个容器，内有256个浮点数据
	//***转换颜色空间
	for (int pixelValue = 0; pixelValue < 256; ++pixelValue) {
		double normalizedValue = pixelValue/255.0;		//0～1（归一化处理）
		double transformedValue = (normalizedValue <= 0.04045) ? normalizedValue/12.92 : pow((normalizedValue+0.055)/1.055, 2.4);		//pow（2，3）表示指数幂即2的3次方

		sRGBGammaCorrections[pixelValue] = static_cast<float>(transformedValue);	//转换后的容器内的值即为后续准备，下标即为0～255
	}
	//***遍历每一个像素点进行转换
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			png::rgb_pixel rgbPixel = leftImage.get_pixel(x, y);		//获取图像中指定位置的RGB像素值
			//对像素值按上述方法转换
